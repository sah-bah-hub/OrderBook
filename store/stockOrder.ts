...
export const useStockOrder = defineStore('stockOrderStore', () => {
    ...
    const closeWwsConection = () => {
        socket.close();
    };
    const openWwsConection = () => {
        // Connect to WebSocket for order book data
        socket = new WebSocket(`wss://stream.binance.com:9443/ws/${store.currentPair.toLocaleLowerCase()}@depth`);
    ...
    return {getDepth, depthData, closeWwsConection ,openWwsConection}
})